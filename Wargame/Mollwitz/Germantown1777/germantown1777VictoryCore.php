...
class germantown1777VictoryCore extends \Wargame\Mollwitz\victoryCore
{
    ...
    public function specialHexChange($args)
    {
        ...
        if (in_array($mapHexName, $battle->specialHexA)) {
            ...
            if ($forceId == LOYALIST_FORCE) {
                $this->victoryPoints[REBEL_FORCE] -= 5;
                $battle->mapData->specialHexesVictory->$mapHexName = "<span class='saxonPolish'>-5 Rebel vp</span>";
            }
        }
        if (in_array($mapHexName, $battle->specialHexB)) {
            if ($forceId == LOYALIST_FORCE) {
                $this->victoryPoints[LOYALIST_FORCE] += 5;
                $battle->mapData->specialHexesVictory->$mapHexName = "<span class='saxonPolish'>+5 Loyalist vp</span>";
            }
            if ($forceId == REBEL_FORCE) {
                $this->victoryPoints[LOYALIST_FORCE] -= 5;
    ...
    protected function checkVictory( $battle)
    {
        ...
        $turn = $gameRules->turn;
        $rebelWin = $saxonPolishWin = $draw = false;

        if (!$this->gameOver) {
            $specialHexes = $battle->mapData->specialHexes;
            $earlyWinScore = 30;
            $winScore = 40;
            if ($this->victoryPoints[REBEL_FORCE] >= $earlyWinScore) {
                if ($turn <= 6) {
                    $rebelWin = true;
                }
            }
            ...
            if ($turn == ($gameRules->maxTurn + 1)) {
                ...
                if(!$saxonPolishWin && !$rebelWin){
                    $this->winner = LOYALIST_FORCE;
                    $gameRules->flashMessages[] = "Loyalist Win";
                    $gameRules->flashMessage[] = "Rebel Fail to Win";
                }
            }
            if($this->gameOver){
    ...
    public function phaseChange()
    {
        ...
        if ($turn >= 2 && $gameRules->phase == RED_MOVE_PHASE && $this->divisionReleased === false){
            ...
            if($this->divisionReleased === false){
                $battle->gameRules->flashMessages[] = "Loyalist Division Not Released $dieRoll";
            }else{
                $battle->gameRules->flashMessages[] = "Loyalist Division Released!!!!!!! $dieRoll";
            }
    ...
    public function postRecoverUnits($args)
    {
        ...
        if ($b->gameRules->turn == 1 && $b->gameRules->phase == BLUE_MOVE_PHASE) {
            $b->gameRules->flashMessages[] = "Rebel Movement allowance +1 this turn.";
        }
        if ($b->gameRules->turn == 1 && $b->gameRules->phase == RED_MOVE_PHASE) {
            $b->gameRules->flashMessages[] = "Loyalist Movement halved this turn.";
        }
    }
