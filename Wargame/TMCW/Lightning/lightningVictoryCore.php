...
class lightningVictoryCore extends \Wargame\TMCW\victoryCore
{
...
//        $ret->scienceCenterDestroyed = $this->scienceCenterDestroyed;
    ...
    public function enterMapSymbol($args)
    {
        ...
        if ($unit->forceId == Lightning::LOYALIST_FORCE) {
            ...
            if(count($this->landingZones) === 0){
                ...
                $this->winner = Lightning::LOYALIST_FORCE;
                $this->gameOver = true;
            }
...
//        if ($mapHexName == 1807 && $forceId == Lightning::REBEL_FORCE) {
...
//        }
        if ($forceId == Lightning::LOYALIST_FORCE) {
            $newLandings = [];
        ...
        if(in_array($mapHexName,$battle->specialHexA)){
            ...
            if ($forceId == Lightning::REBEL_FORCE) {
                $this->victoryPoints[Lightning::REBEL_FORCE]  += $vp;
                $battle->mapData->specialHexesVictory->$mapHexName = "<span class='rebelVictoryPoints'>+$vp Rebel vp</span>";
//                $this->victoryPoints[Lightning::LOYALIST_FORCE] -= $vp;
...
//                $theUnits[$id]->status = STATUS_ELIMINATED;
                ...
                if($unit->forceId === Lightning::REBEL_FORCE){
                    if($unit->class === 'para'){
...
//
//        if($rebelOption === 0){
//            if($battle->mapData->getSpecialHex($battle->specialHexA[0]) === Lightning::REBEL_FORCE){
//                $this->victoryPoints[Lightning::REBEL_FORCE] += 25;
...
//        }
//        if($rebelOption === 1){
//            if($battle->mapData->getSpecialHex($battle->specialHexB[0]) === Lightning::REBEL_FORCE){
//                $this->victoryPoints[Lightning::REBEL_FORCE] += 25;
//                $battle->gameRules->flashMessages[] = "Rebel Player Completes Goal";
//            }else{
//                $battle->gameRules->flashMessages[] = "Rebel Player FAILS Goal";
...
//        }
//        if($rebelOption === 2){
//            if($battle->mapData->getSpecialHex($battle->specialHexC[0]) === Lightning::REBEL_FORCE){
//                $this->victoryPoints[Lightning::REBEL_FORCE] += 25;
//                $battle->gameRules->flashMessages[] = "Rebel Player Completes Goal";
//            }else{
//                $battle->gameRules->flashMessages[] = "Rebel Player FAILS Goal";
...
//            }
//        }
        if ($this->victoryPoints[Lightning::LOYALIST_FORCE] > $this->victoryPoints[Lightning::REBEL_FORCE]) {
            $battle->gameRules->flashMessages[] = "Loyalist Player Wins";
            $this->winner = Lightning::LOYALIST_FORCE;
        }
        if ($this->victoryPoints[Lightning::REBEL_FORCE] > $this->victoryPoints[Lightning::LOYALIST_FORCE]) {
            $battle->gameRules->flashMessages[] = "Rebel Player Wins";
            $this->winner = Lightning::REBEL_FORCE;
        }
        if ($this->victoryPoints[Lightning::LOYALIST_FORCE] == $this->victoryPoints[Lightning::REBEL_FORCE]) {
...
//            return;
    ...
    public function playerTurnChange($arg)
    {
        ...
        if($gameRules->turn <= $gameRules->maxTurn) {
            if ($attackingId == Lightning::REBEL_FORCE) {
                $gameRules->flashMessages[] = "Rebel Player Turn";
            }
            if ($attackingId == Lightning::LOYALIST_FORCE) {
