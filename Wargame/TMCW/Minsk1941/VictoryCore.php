...
class VictoryCore extends \Wargame\TMCW\victoryCore
{
    ...
    public function specialHexChange($args)
    {
        ...
        if(in_array($mapHexName, $battle->specialHexA)){
            ...
            if ($forceId == Minsk1941::SOVIET_FORCE) {
                $battle->mapData->specialHexesVictory->$mapHexName = "<span class='soviet'>-10 German VP</span>";
                $this->victoryPoints[Minsk1941::GERMAN_FORCE]  -= 10;
            }
            if ($forceId == Minsk1941::GERMAN_FORCE) {
                $battle->mapData->specialHexesVictory->$mapHexName = "<span class='german'>+10 German VP</span>";
                $this->victoryPoints[Minsk1941::GERMAN_FORCE]  += 10;
            }
        }
        if(in_array($mapHexName, $battle->specialHexB)){
            if ($forceId == Minsk1941::SOVIET_FORCE) {
                $battle->mapData->specialHexesVictory->$mapHexName = "<span class='soviet'>-10 German VP</span>";
                $this->victoryPoints[Minsk1941::GERMAN_FORCE]  -= 25;
            }
            if ($forceId == Minsk1941::GERMAN_FORCE) {
                $battle->mapData->specialHexesVictory->$mapHexName = "<span class='german'>+10 German VP</span>";
                $this->victoryPoints[Minsk1941::GERMAN_FORCE]  += 25;
    ...
    public function postReinforceZones($args)
    {
        ...
        if($unit->forceId == Minsk1941::GERMAN_FORCE){
    ...
    public function checkSurrounded(){
        ...
        if(!$this->gameOver){
            ...
            foreach($units as $unit){
                if($unit->forceId == Minsk1941::SOVIET_FORCE){
    ...
    public function gameEnded()
    {
        ...
        if($this->victoryPoints[Minsk1941::GERMAN_FORCE] > 49){
            $this->winner = Minsk1941::GERMAN_FORCE;
            $this->gameOver = true;
            $battle->gameRules->flashMessages[] = "Germans Wins";
        }else{
            $this->winner = Minsk1941::SOVIET_FORCE;
            $this->gameOver = true;
